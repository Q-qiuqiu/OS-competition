...
# set top k for each class
#top_k = {'airplane':600, 'bridge':350, 'car':1900, 'oiltank':250, 'playground':2000, 'port':170, 'ship':750}
...
s = ('%10s' + '%20s' * 3) % ('Class', 'Precision', 'Recall','AP')
print(s)
...
def voc_ap(rec, prec):
  ...
  """
   This part makes the precision monotonically decreasing
    (goes from the end to the beginning)下 find / -name "filname".txt
...
with open(results_files_path + "/results.txt", 'w') as results_file:
  ...
  for class_index, class_name in enumerate(gt_classes):
    ...
    predictions_data = json.load(open(predictions_file))#按置信度从大到小排序的

    #k = top_k[class_name]
    #predictions_data = predictions_data[:k]
    """
    ...
    for idx, prediction in enumerate(predictions_data):
      ...
      conf.append(float(prediction["confidence"]))
      if show_animation:
    ...
    prec = tp[:]
    for idx, val in enumerate(tp):
      prec[idx] = float(tp[idx]) / (fp[idx] + tp[idx]) #这个类所有框的precision
    #print(prec)
    rec_conf = np.interp(-pr_score,-np.array(conf),np.array(rec))#每个类的recall 阈值大于pr_score
    pre_conf = np.interp(-pr_score,-np.array(conf),np.array(prec)) #每个类的precision 阈值大于pr_score
    ap, mrec, mprec = voc_ap(rec, prec)
    ss = ('%10s' + '%20s' * 3) % (class_name, pre_conf, rec_conf,ap)
    print(ss)
    sum_AP += ap
    sum_P+=pre_conf
    sum_R+=rec_conf
  ...
  results_file.write(text + "\n")
  #print(text)
  final = ('%10s' + '%20s' * 3) % ('all', sum_P/n_classes, sum_R/n_classes,mAP)
  print(final)
